 ...
 */
error NOT_OWNER();
error INVALID_NUM_SIGNERS();
error NOT_UNIQUE_OWNER();
error NOT_A_TRANSACTION();

contract CustomAccount is
    BaseAccount,
    ...
    Initializable
{
    ...
    event CustomAccountInitialized(
    ...
    modifier requireFromEntryPointOrOwner() {
        if (!(msg.sender == address(entryPoint())) || !isOwner[msg.sender]) {
            revert NOT_OWNER();
        }
        _;
    }
    ...
    ) external requireFromThisOrOwner {
        transactions.push(
    ...
    function execute(
        uint _txId
    ) external requireFromThisOrOwner txExists(_txId) notExecuted(_txId) {
        require(
            getApprovalCount(_txId) >= requiredSigners,
            "not enough signers"
        );

        Transaction storage txn = transactions[_txId];
        _call(payable(txn.to), txn.value, txn.data);
        txn.executed = true;
    ...
    ) internal view override requireFromThisOrOwner {
        (newImplementation);
    }
}
